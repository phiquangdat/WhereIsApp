...
const RecordEditScreen: React.FC<Props> = ({navigation}) => {
    ...
    const [location, setLocation] = useState<{ latitude: number; longitude: number } | null>(note.location);
    ...
    const handleSave = async () => {
        ...
        try {
            ...
            const updatedNotes = notes.map(n => (n.id === note.id ? { ...n, name, description, image, location } : n));
            await AsyncStorage.setItem('records', JSON.stringify(updatedNotes));

            Alert.alert("Success", "Note updated successfully!", [
                { text: "OK", onPress: () => navigation.goBack({note: note}) }
            ]);
        } catch (error) {
